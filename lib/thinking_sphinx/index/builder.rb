module ThinkingSphinx
  class Index
    ...
    class Builder
      ...
      # Use this method to add custom JOINS (or other SQL) to your association list
      # You can call this method multiple times and all queries will be merged into
      # the big index query
      #
      #   from "LEFT OUTER JOIN `items` ON (`products`.id = `items`.product_id)"
      #
      def from(sql)
        source.froms << ThinkingSphinx::SimpleAssociation.new(sql)
      end
